...
void UIOverlay::draw(const VkCommandBuffer commandBuffer,
  ...
  for (int32_t i = 0; i < imDrawData->CmdListsCount; i++) {
    ...
    TARGET_OS_SIMULATOR
    // Apple Device Simulator does not support vkCmdDrawIndexed() with
    // vertexOffset > 0, so rebind vertex buffer instead
    offsets[0] += cmd_list->VtxBuffer.Size * sizeof(ImDrawVert);
    vkCmdBindVertexBuffers(commandBuffer, 0, 1,
                           &buffers[currentBuffer].vertexBuffer.buffer,
                           offsets);
#else
...
bool UIOverlay::radioButton(const char* label, int* v, int value) {
  return ImGui::RadioButton(label, v, value);
}
