 ...
 *
 * This sample shows how to use host image copies to directly upload an image to
 * the devic without having to use staging
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  /*
          ...
          Unlike the texture(3d/array/etc) samples, this one uses the
     VK_EXT_host_image_copy to drasticly simplify the process of uploading an
     image from the host to the GPU. This new extension adds a way of directly
     uploading image data from host memory to an optimal tiled image on the
     device (GPU). This no longer requires a staging buffer in between, as we
     can now directly copy data stored in host memory to the image. The
     extension also adds new functionality to simplfy image barriers
  */
  void loadTexture() {
    ...
    // The flag we need to check is an extension flag, so we need to go through
    // VkFormatProperties3
    ...
    if ((formatProperties3.optimalTilingFeatures &
      ...
      vks::tools::exitFatal(
          "The selected image format does not support the required host "
          "transfer bit.",
          -1);
    }
    ...
    // For images that use host image copy we need to specify the
    // VK_IMAGE_USAGE_HOST_TRANSFER_BIT_EXT usage flag
    ...
    // already tightly packed, doesn't need and swizzle and as such matches what
    // the device expects
    ...
    for (uint32_t i = 0; i < texture_.mipLevels; i++) {
      ...
      // As the KTX file is tightly packed, we can simply offset into that
      // buffer for the current mip level
      ktx_size_t offset;
      KTX_error_code ret =
    ...
    // VK_EXT_host_image_copy als introduces a simplified way of doing the
    // required image transition on the host This no longer requires a dedicated
    // command buffer to submit the barrier We also no longer need multiple
    // transitions, and only have to do one for the final layout
    VkHostImageLayoutTransitionInfoEXT hostImageLayoutTransitionInfo{};
    hostImageLayoutTransitionInfo.sType =
    ...
    hostImageLayoutTransitionInfo.newLayout =
    ...
    // The implementation will then convert this to an implementation specific
    // optimal tiling layout
