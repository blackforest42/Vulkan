...
public:
	...
	void loadAssets()
	{
		...
		textures_.normalHeightMap.loadFromFile(getAssetPath() + "textures/rocks_normal_height_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
		textures_.colorMap.loadFromFile(getAssetPath() + "textures/rocks_color_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
	}
	...
	void updateUniformBuffers()
	{
		...
		memcpy(uniformBuffers_[currentBuffer].vertexShader.mapped, &uniformDataVertexShader, sizeof(UniformDataVertexShader));

		// Fragment shader
		memcpy(uniformBuffers_[currentBuffer].fragmentShader.mapped, &uniformDataFragmentShader, sizeof(UniformDataFragmentShader));
	}
