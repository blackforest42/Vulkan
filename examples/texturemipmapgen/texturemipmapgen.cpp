 ...
 *
 * This samples shows how to generate a full mip-chain from a top-level image
 * and how different sampling modes compare
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // Loads a full sized image from disk, generates a Vulkan image (texture) from
  // it and creates a full mip chain using blits
  void loadTextureAndGenerateMips(std::string filename, VkFormat format) {
    ...
    texture_.mipLevels = static_cast<uint32_t>(
        floor(log2(std::max(texture_.width, texture_.height))) + 1);
    ...
    vks::tools::insertImageMemoryBarrier(
        copyCmd, texture_.image, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
    ...
    vks::tools::insertImageMemoryBarrier(
        copyCmd, texture_.image, VK_ACCESS_TRANSFER_WRITE_BIT,
    ...
    VkCommandBuffer blitCmd = vulkanDevice_->createCommandBuffer(
        VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    ...
    for (uint32_t i = 1; i < texture_.mipLevels; i++) {
      ...
      vks::tools::insertImageMemoryBarrier(
          blitCmd, texture_.image, 0, VK_ACCESS_TRANSFER_WRITE_BIT,
          VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
          VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
          mipSubRange);

      // Blit from previous level
      vkCmdBlitImage(blitCmd, texture_.image,
                     VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, texture_.image,
                     VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &imageBlit,
                     VK_FILTER_LINEAR);

      // Prepare current mip level as image blit source for next level
      vks::tools::insertImageMemoryBarrier(
          blitCmd, texture_.image, VK_ACCESS_TRANSFER_WRITE_BIT,
          VK_ACCESS_TRANSFER_READ_BIT, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
          VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_PIPELINE_STAGE_TRANSFER_BIT,
    ...
    vks::tools::insertImageMemoryBarrier(
        blitCmd, texture_.image, VK_ACCESS_TRANSFER_READ_BIT,
    ...
    // Create some samplers with different settings that can be selected via the
    // UI
    samplers.resize(3);
  ...
  void loadAssets() {
    model.loadFromFile(getAssetPath() + "models/tunnel_cylinder.gltf",
                       vulkanDevice_, queue_,
                       vkglTF::FileLoadingFlags::PreTransformVertices |
                           vkglTF::FileLoadingFlags::FlipY);
    loadTextureAndGenerateMips(
  ...
  void setupDescriptors() {
    ...
    VkDescriptorImageInfo textureDescriptor =
        vks::initializers::descriptorImageInfo(
            VK_NULL_HANDLE, texture_.view,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    // Sets per frame, just like the buffers themselves
    // Images and samplers do not need to be duplicated per frame, we reuse the
    // same one for each frame
    ...
    for (auto i = 0; i < uniformBuffers_.size(); i++) {
      ...
      std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
          ...
          // Binding 1: Sampled image
          vks::initializers::writeDescriptorSet(
              descriptorSets_[i], VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, 1,
              &textureDescriptor)};

      // Binding 2: Contains an array of samplers that can be switched from the
      // UI to demonstrate different filteirng modes
      std::vector<VkDescriptorImageInfo> samplerDescriptors;
      for (auto j = 0; j < samplers.size(); j++) {
        samplerDescriptors.push_back(vks::initializers::descriptorImageInfo(
            samplers[j], VK_NULL_HANDLE,
            VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL));
      }
  ...
  void preparePipelines() {
    ...
    shaderStages[0] =
        loadShader(getShadersPath() + "texturemipmapgen/texture.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getShadersPath() + "texturemipmapgen/texture.frag.spv",
  ...
  void updateUniformBuffers() {
    ...
    uniformData_.model =
        glm::rotate(glm::mat4(1.0f), glm::radians(timer * 360.0f),
                    glm::vec3(1.0f, 0.0f, 0.0f));
    uniformData_.viewPos =
        glm::vec4(camera_.position_, 0.0f) * glm::vec4(-1.0f);
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay* overlay) {
    if (overlay->header("Settings")) {
      if (overlay->sliderFloat("LOD bias", &uniformData_.lodBias, 0.0f,
                               (float)texture_.mipLevels)) {
        updateUniformBuffers();
      }
      if (overlay->comboBox("Sampler type", &uniformData_.samplerIndex,
                            samplerNames)) {
        updateUniformBuffers();
      }
    }
