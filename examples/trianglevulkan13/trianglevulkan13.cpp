 ...
 *
 * Note:
 * This is a variation of the the triangle sample that makes use of Vulkan 1.3
 * features This simplifies the api a bit, esp. with dynamic rendering replacing
 * render passes (and with that framebuffers)
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // away. Getting this right is crucial to using Vulkan. Semaphores are used to
  // coordinate operations within the graphics queue and ensure correct command
  // ordering
  ...
  ~VulkanExample() override {
    ...
    if (device) {
      ...
      for (uint32_t i = 0; i < maxConcurrentFrames; i++) {
        ...
        vkDestroyBuffer(device, uniformBuffers_[i].handle, nullptr);
  ...
  virtual void getEnabledFeatures() override {
    ...
    if (deviceProperties.apiVersion < VK_API_VERSION_1_3) {
      vks::tools::exitFatal("Selected GPU does not support support Vulkan 1.3",
  ...
  // Create the per-frame (in flight) and per (swapchain image) Vulkan
  // synchronization primitives used in this example
  ...
  // Command buffers are used to record commands to and are submitted to a queue
  ...
  void createCommandBuffers() {
    ...
    VkCommandBufferAllocateInfo cmdBufAllocateInfo =
        vks::initializers::commandBufferAllocateInfo(
            commandPool, VK_COMMAND_BUFFER_LEVEL_PRIMARY, maxConcurrentFrames);
  ...
  void createVertexBuffer() {
    ...
    //	This is a complex topic and while it's fine for an example application
    // to small individual memory allocations that is not 	what should be
    // done a real-world application, where you should allocate large chunks of
    // memory at once instead.
    ...
    // Setup indices
    // We do this for demonstration purposes, a triangle doesn't require indices
    // to be rendered (because of the 1:1 mapping), but more complex shapes
    // usually make use of indices
    ...
    // Create the host visible staging buffer that we copy vertices and indices
    // too, and from which we copy to the device
    ...
    VK_CHECK_RESULT(vkCreateBuffer(device, &stagingBufferCI, nullptr,
    ...
    VK_CHECK_RESULT(
        vkAllocateMemory(device, &memAlloc, nullptr, &stagingBuffer.memory));
    VK_CHECK_RESULT(vkBindBufferMemory(device, stagingBuffer.handle,
                                       stagingBuffer.memory, 0));
    // Map the buffer and copy vertices and indices into it, this way we can use
    // a single buffer as the source for both vertex and index GPU buffers
    uint8_t* data{nullptr};
    VK_CHECK_RESULT(vkMapMemory(device, stagingBuffer.memory, 0,
                                memAlloc.allocationSize, 0, (void**)&data));
    ...
    vertexbufferCI.usage =
        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    vertexbufferCI.size = vertexBufferSize;
    VK_CHECK_RESULT(
        vkCreateBuffer(device, &vertexbufferCI, nullptr, &vertexBuffer.handle));
    ...
    VK_CHECK_RESULT(vkBindBufferMemory(device, vertexBuffer.handle,
                                       vertexBuffer.memory, 0));

    // Create a device local buffer to which the (host local) index data will be
    // copied and which will be used for rendering
    VkBufferCreateInfo indexbufferCI{VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO};
    indexbufferCI.usage =
        VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT;
    indexbufferCI.size = indexBufferSize;
    VK_CHECK_RESULT(
        vkCreateBuffer(device, &indexbufferCI, nullptr, &indexBuffer.handle));
    vkGetBufferMemoryRequirements(device, indexBuffer.handle, &memReqs);
    memAlloc.allocationSize = memReqs.size;
    memAlloc.memoryTypeIndex = getMemoryTypeIndex(
        memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    VK_CHECK_RESULT(
        vkAllocateMemory(device, &memAlloc, nullptr, &indexBuffer.memory));
    VK_CHECK_RESULT(
        vkBindBufferMemory(device, indexBuffer.handle, indexBuffer.memory, 0));
    ...
    vkCmdCopyBuffer(copyCmd, stagingBuffer.handle, vertexBuffer.handle, 1,
                    &copyRegion);
    copyRegion.size = indexBufferSize;
    // Indices are stored after the vertices in the source buffer, so we need to
    // add an offset
    copyRegion.srcOffset = vertexBufferSize;
    vkCmdCopyBuffer(copyCmd, stagingBuffer.handle, indexBuffer.handle, 1,
                    &copyRegion);
    ...
    // The fence made sure copies are finished, so we can safely delete the
    // staging buffer
    vkDestroyBuffer(device, stagingBuffer.handle, nullptr);
    vkFreeMemory(device, stagingBuffer.memory, nullptr);
  }

  // Decriptors are used to pass data to shaders, for our sample we use a
  // descriptor to pass parameters like matrices to the shader
  void createDescriptors() {
    ...
    // Descriptor set layouts define the interface between our application and
    // the shader Basically connects the different shader stages to descriptors
    // for binding uniform buffers, image samplers, etc. So every shader binding
    // should map to one descriptor set layout binding Binding 0: Uniform buffer
    // (Vertex shader)
    ...
    // Where the descriptor set layout is the interface, the descriptor set
    // points to actual data Descriptors that are changed per frame need to be
    // multiplied, so we can update descriptor n+1 while n is still used by the
  ...
  // While we don't do any depth testing in this sample, having depth testing is
  // very common so it's a good idea to learn it from the very start
  ...
  void createPipeline() {
    // The pipeline layout is the interface telling the pipeline what type of
    // descriptors will later be bound
    ...
    // stored and hashed on the GPU making pipeline changes very fast
    ...
    // Most states are baked into the pipeline, but there is somee state that
    // can be dynamically changed within the command buffer to mak e things
    // easuer To be able to change these we need do specify which dynamic states
    // will be changed using this pipeline. Their actual states are set later on
  ...
  void createUniformBuffers() {
    ...
    for (uint32_t i = 0; i < maxConcurrentFrames; i++) {
      VK_CHECK_RESULT(vkCreateBuffer(device, &bufferInfo, nullptr,
                                     &uniformBuffers_[i].handle));
      // Get memory requirements including size, alignment and memory type based
      // on the buffer type we request (uniform buffer)
      VkMemoryRequirements memReqs;
      vkGetBufferMemoryRequirements(device, uniformBuffers_[i].handle,
                                    &memReqs);
      VkMemoryAllocateInfo allocInfo{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
      // Note that we use the size we got from the memory requirements and not
      // the actual buffer size, as the former may be larger due to alignment
      ...
      VK_CHECK_RESULT(vkBindBufferMemory(device, uniformBuffers_[i].handle,
  ...
  void render() override {
    ...
    // Copy the current matrices to the current frame's uniform buffer. As we
    // requested a host coherent memory type for the uniform buffer, the write
    // is instantly visible to the GPU.
    ...
    // With dynamic rendering we need to explicitly add layout transitions by
    // using barriers, this set of barriers prepares the color and depth images
    // for output
    vks::tools::insertImageMemoryBarrier(
        commandBuffer, swapChain.images[imageIndex], 0,
    ...
    vks::tools::insertImageMemoryBarrier(
        commandBuffer, depthStencil.image, 0,
  ...
  // Override these as otherwise the base class would generate frame buffers and
  // render passes
