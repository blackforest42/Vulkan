 ...
 *
 * This sample shows how to read data from a buffer device address (aka
 * "reference") instead of using uniforms The application passes buffer device
 * addresses to the shader via push constants, and the shader then simply reads
 * the data behind that address This makes stuff like descriptor handling a lot
 * easier, as we now only pass around references akin to pointers and no longer
 * need descriptors for buffers See cube.vert for the shader side of things
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // This sample passes the buffer references ("pointer") using push constants,
  // the shader then reads data from that buffer address
  ...
  void preparePipelines() {
    ...
    // That way it's very easy to do a draw call, change the reference to
    // another buffer (or part of that buffer) and do the next draw call using
    // different data
    ...
    std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages = {
        loadShader(getShadersPath() + "bufferdeviceaddress/cube.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT),
        loadShader(getShadersPath() + "bufferdeviceaddress/cube.frag.spv",
  ...
  void prepareUniformBuffers() {
    for (uint32_t i = 0; i < maxConcurrentFrames; i++) {
      ...
      // Get the device of this buffer that is later on passed to the shader
      // (aka "reference")
      VkBufferDeviceAddressInfo bufferDeviceAdressInfo{};
      bufferDeviceAdressInfo.sType =
      ...
      for (auto& cube : cubes) {
        ...
        VK_CHECK_RESULT(vulkanDevice->createBuffer(
            ...
            VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
            &cube.buffers[i], sizeof(glm::mat4)));
  ...
  void updateUniformBuffers() {
    ...
    for (auto& cube : cubes) {
      cube.modelMatrix =
          glm::rotate(cube.modelMatrix, glm::radians(cube.rotation.x),
                      glm::vec3(1.0f, 0.0f, 0.0f));
      cube.modelMatrix =
          glm::rotate(cube.modelMatrix, glm::radians(cube.rotation.y),
  ...
  void prepare() {
    ...
    // We need this extension function to get the address of a buffer so we can
  ...
  void buildCommandBuffer() {
    ...
    // Instead of using descriptors to pass global and per-model matrices to the
    // shader, we can now simply pass buffer references via push constants The
    ...
    for (auto& cube : cubes) {
      ...
      // So instead of having to bind different descriptors, we only pass a
