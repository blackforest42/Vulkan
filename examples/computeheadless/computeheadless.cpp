...
class VulkanExample {
 public:
  ...
  VulkanExample() {
    ...
    // Shaders generated by Slang require a certain SPIR-V environment that
    // can't be satisfied by Vulkan 1.0, so we need to expliclity up that to at
    ...
    */
...
#endif
#if (defined(VK_USE_PLATFORM_MACOS_MVK) || defined(VK_USE_PLATFORM_METAL_EXT))
    // SRS - When running on macOS with MoltenVK, enable
    // VK_KHR_get_physical_device_properties2 (required by
    // VK_KHR_portability_subset)
    instanceExtensions.push_back(
        VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
#if defined(VK_KHR_portability_enumeration)
    // SRS - When running on macOS with MoltenVK and
    ...
    if (instanceExtCount > 0) {
      ...
      if (vkEnumerateInstanceExtensionProperties(
              nullptr, &instanceExtCount, &extensions.front()) == VK_SUCCESS) {
    ...
    */
    ...
    // When running on macOS with MoltenVK and VK_KHR_portability_subset is
    // defined and supported by the device, enable the extension
    ...
    if (deviceExtCount > 0) {
      ...
      if (vkEnumerateDeviceExtensionProperties(
              physicalDevice, nullptr, &deviceExtCount, &extensions.front()) ==
          VK_SUCCESS) {
        for (VkExtensionProperties extension : extensions) {
          if (strcmp(extension.extensionName,
    ...
    */
    ...
    // Copy input data to VRAM using a staging buffer
    {
      createBuffer(
          ...
          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &hostBuffer, &hostMemory,
    ...
    */
    {
      ...
      const std::string shadersPath =
          getShaderBasePath() + shaderDir + "/computeheadless/";
      ...
      shaderStage.module = vks::tools::loadShader(
          ...
          (shadersPath + "headless.comp.spv").c_str(), device);
#else
      shaderStage.module = vks::tools::loadShader(
          (shadersPath + "headless.comp.spv").c_str(), device);
#endif
...
void android_main(android_app* state) {
  ...
  while ((ident = ALooper_pollOnce(-1, NULL, &events, (void**)&source)) >
...
int main(int argc, char* argv[]) {
  ...
  commandLineParser.add("shaders", {"-s", "--shaders"}, 1,
                        "Select shader type to use (glsl, hlsl or slang)");
