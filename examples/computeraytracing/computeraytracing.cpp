 ...
 *
 * This samples implements a basic ray tracer with materials and reflections
 * using a compute shader Shader storage buffers are used to pass geometry
 * information for spheres and planes to the computer shader The compute shader
 * then uses these as the scene geometry for ray tracing and outputs the results
 * to a storage image The graphics part of the sample then displays that image
 * full screen Not to be confused with actual hardware accelerated ray tracing
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  // Resources for the graphics part of the example. The graphics pipeline
  // simply displays the compute shader output
  struct Graphics {
  ...
  struct Compute {
    VkQueue queue{
        VK_NULL_HANDLE};  // Separate queue for compute commands (queue family
    ...
    VkDescriptorSetLayout descriptorSetLayout{
        VK_NULL_HANDLE};  // Compute shader binding layout
    VkPipelineLayout pipelineLayout{
        VK_NULL_HANDLE};                  // Layout of the compute pipeline
    ...
    // Object properties for planes and spheres are passed via a shade storage
    // buffer There is no vertex data, the compute shader calculates the
  ...
  // The sample uses spheres and planes that are passed to the compute shader
  // via a shader storage buffer The computer shader uses the object type to
  // select different calculations
  enum class SceneObjectType { Sphere = 0, Plane = 1 };
  // Spheres and planes are described by different properties, we use a union
  // for this
  ...
  struct SceneObject {
    ...
    // Due to alignment rules we need to pad to make the element align at
    // 16-bytes
    glm::ivec2 _pad;
  };
  ...
  // Prepare a storage image that is used to store the compute shader ray
  // tracing output
  void prepareStorageImage() {
    ...
    vks::tools::setImageLayout(
        layoutCmd, storageImage.image, VK_IMAGE_ASPECT_COLOR_BIT,
        VK_IMAGE_LAYOUT_UNDEFINED, storageImage.imageLayout);
  ...
  // definitions for the raytraced scene
  void prepareStorageBuffers() {
    ...
    auto addSphere = [&sceneObjects, &currentId](glm::vec3 pos, float radius,
                                                 glm::vec3 diffuse,
                                                 float specular) {
      SceneObject sphere{};
      sphere.id = currentId++;
    ...
    auto addPlane = [&sceneObjects, &currentId](
                        glm::vec3 normal, float distance, glm::vec3 diffuse,
                        float specular) {
      SceneObject plane{};
      plane.id = currentId++;
    ...
    addSphere(glm::vec3(1.75f, -0.5f, 0.0f), 1.0f, glm::vec3(0.0f, 1.0f, 0.0f),
              32.0f);
    addSphere(glm::vec3(0.0f, 1.0f, -0.5f), 1.0f,
              glm::vec3(0.65f, 0.77f, 0.97f), 32.0f);
    addSphere(glm::vec3(-1.75f, -0.75f, -0.5f), 1.25f,
    ...
    addPlane(glm::vec3(-1.0f, 0.0f, 0.0f), roomDim, glm::vec3(1.0f, 0.0f, 0.0f),
             32.0f);
    addPlane(glm::vec3(1.0f, 0.0f, 0.0f), roomDim, glm::vec3(0.0f, 1.0f, 0.0f),
             32.0f);
  ...
  void prepareGraphics() {
    ...
    // Binding 0: Storage image with raytraced output as a sampled image for
    // displaying it
    ...
    shaderStages[0] =
        loadShader(getShadersPath() + "computeraytracing/texture.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] =
        loadShader(getShadersPath() + "computeraytracing/texture.frag.spv",
  ...
  void prepareCompute() {
    ...
    // duplicated per frame, we reuse the same one for each frame
    for (auto i = 0; i < compute.uniformBuffers.size(); i++) {
      ...
      std::vector<VkWriteDescriptorSet> computeWriteDescriptorSets = {
          vks::initializers::writeDescriptorSet(
              compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, 0,
              &storageImage.descriptor),
          vks::initializers::writeDescriptorSet(
              compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 1,
              &compute.uniformBuffers[i].descriptor),
          vks::initializers::writeDescriptorSet(
              compute.descriptorSets[i], VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 2,
  ...
  void updateUniformBuffers() {
    ...
    compute.uniformData.lightPos.y =
