/*
 * Vulkan Example - Example for using printf in shaders to help debugging. Can
 * be used in conjunction with a debugging app like RenderDoc
 * (https://renderdoc.org)
 *
...
/*
 * The only change required for printf in shaders on the application is enabling
 * the VK_KHR_shader_non_semantic_info extensions The actual printing is done in
 * the shaders (see toon.vert from the glsl/hlsl) folder For glsl shaders that
 * use this feature, the GL_EXT_debug_printf extension needs to be enabled
 */
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  VulkanExample() : VulkanExampleBase() {
    title_ = "Debug output with shader printf";
    ...
    // SRS - Force validation on since shader printf provided by
    // VK_LAYER_KHRONOS_validation on macOS
    ...
    // Make static so layer setting reference remains valid after leaving
    // constructor scope
    static const char* layerEnables =
        "VK_VALIDATION_FEATURE_ENABLE_DEBUG_PRINTF_EXT";
    layerSetting.pValues = &layerEnables;
    enabledLayerSettings.push_back(layerSetting);

    // SRS - RenderDoc not available on macOS so redirect printf output to
    // stdout
  ...
  void preparePipelines() {
    ...
    std::array<VkPipelineShaderStageCreateInfo, 2> shaderStages = {
        loadShader(getShadersPath() + "debugprintf/toon.vert.spv",
                   VK_SHADER_STAGE_VERTEX_BIT),
        loadShader(getShadersPath() + "debugprintf/toon.frag.spv",
