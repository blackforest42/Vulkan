 ...
 *
 * This sample demonstrates the use of some of the VK_EXT_dynamic_state
 * extensions These allow an application to set some pipeline related state
 * dynamically at drawtime instead of having to pre-bake the state into a
 * pipeline This can help reduce the number of pipelines required
 *
...
class VulkanExample : public VulkanExampleBase {
 public:
  ...
  PFN_vkCmdSetRasterizerDiscardEnable vkCmdSetRasterizerDiscardEnableEXT{
      nullptr};
  ...
  void getEnabledExtensions() {
    // Get the full list of extended dynamic state features supported by the
    // device
    extendedDynamicStateFeaturesEXT.sType =
        VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_EXTENDED_DYNAMIC_STATE_FEATURES_EXT;
    extendedDynamicStateFeaturesEXT.pNext = &extendedDynamicState2FeaturesEXT;
    extendedDynamicState2FeaturesEXT.sType =
  ...
  void preparePipelines() {
    ...
    // Pipeline
    // Instead of having to create a pipeline for each state combination, we
    // only create one pipeline and toggle the new dynamic states during command
    // buffer creation
    ...
    shaderStages[0] = loadShader(getShadersPath() + "pipelines/phong.vert.spv",
  ...
  void prepare() {
    ...
    // Dynamic states are set with vkCmd* calls in the command buffer, so we
  ...
  void buildCommandBuffer() {
    ...
    if (vkCmdSetDepthTestEnableEXT) {
      vkCmdSetDepthTestEnableEXT(cmdBuffer,
                                 VkFrontFace(dynamicState.depthTest));
    }
    if (vkCmdSetDepthWriteEnableEXT) {
      vkCmdSetDepthWriteEnableEXT(cmdBuffer,
                                  VkFrontFace(dynamicState.depthWrite));
    }

    if (vkCmdSetRasterizerDiscardEnableEXT) {
      vkCmdSetRasterizerDiscardEnableEXT(
          cmdBuffer, VkBool32(dynamicState2.rasterizerDiscardEnable));
    }

    if (vkCmdSetColorBlendEnableEXT) {
      const std::vector<VkBool32> blendEnables = {
          dynamicState3.colorBlendEnable};
  ...
  virtual void OnUpdateUIOverlay(vks::UIOverlay* overlay) {
    if (overlay->header("Dynamic state")) {
      if (hasDynamicState) {
        overlay->comboBox("Cull mode", &dynamicState.cullMode,
                          {"none", "front", "back"});
        overlay->comboBox("Front face", &dynamicState.frontFace,
    ...
    if (overlay->header("Dynamic state 2")) {
      if (hasDynamicState2) {
        overlay->checkBox("Rasterizer discard",
                          &dynamicState2.rasterizerDiscardEnable);
      } else {
        overlay->text("Extension or features not supported");
      }
    }
